...
from slime.utils.profiler import (
    INFERENCE_EVENT_DECODE,
    INFERENCE_EVENT_PREFILL,
    INFERENCE_EVENT_UNIFIED,
...
def _record_inference_profile_if_present(
    args: Namespace,
    state: "GenerateState",
    ...
    prefill_start = mi.get("prefill_start_sec", request_start_sec)
    prefill_end = mi.get("prefill_end_sec")
    decode_start = mi.get("decode_start_sec")
    decode_end = mi.get("decode_end_sec")
    unified_start = mi.get("unified_start_sec")
    unified_end = mi.get("unified_end_sec")
    ...
    if decode_end is None and total_t is not None:
    ...
    if prefill_end is not None and prefill_start is not None:
        ...
        prefill_per_rank[gpu_id] = [(prefill_start, prefill_end)]
        events[INFERENCE_EVENT_PREFILL] = prefill_per_rank
    if decode_start is not None and decode_end is not None:
        decode_per_rank = [[] for _ in range(num_gpus)]
        decode_per_rank[gpu_id] = [(decode_start, decode_end)]
        events[INFERENCE_EVENT_DECODE] = decode_per_rank
    if unified_start is not None and unified_end is not None:
        unified_per_rank = [[] for _ in range(num_gpus)]
        unified_per_rank[gpu_id] = [(unified_start, unified_end)]
        events[INFERENCE_EVENT_UNIFIED] = unified_per_rank
    if not events:
        return
